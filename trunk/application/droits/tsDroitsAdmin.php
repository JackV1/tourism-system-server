	...
	class tsDroitsAdmin extends tsDroitsDefault implements tsDroitsInterface
	{
		...
		protected function loadUtilisateursAdministrables()
		{
			...
			*/
			...
			$idFiches = implode(',', $this -> fichesAdministrables);
			$idGroupe = $this -> idGroupe;
			
			/*$keyMemCache = 'USER_ADMIN' . $idGroupe . md5($idFiches);
			$utilisateurs = tsCache::get($keyMemCache);
			
			if($utilisateurs === false)
			{*/
				$utilisateurs = array();
				
				$query = "SELECT DISTINCT su.idUtilisateur FROM sitUtilisateur su, sitUtilisateurDroitFiche sud WHERE su.idGroupe='$idGroupe' AND su.idUtilisateur=sud.idUtilisateur AND sud.idFiche IN('$idFiches')";
				
				$results = tsDatabase::getRows($query, array());
				
				foreach($results as $result)
				{
					$utilisateurs[] = $result['idUtilisateur'];	
				}
				/*
				tsCache::set($keyMemCache, $utilisateurs, 600);
			}*/
